<?php
    $payload = [
        'type'    => $success ? 'gsc_connected' : 'gsc_error',
        'site'    => $site_url ?? null,
        'email'   => $email ?? null,
...
<head>
<meta charset="UTF-8">
<title>{{ $success ? 'Search Console connected' : 'Connection failed' }}</title>
<style>
...
@if($success)
  ...
  <p>{{ !empty($site_url) ? $site_url : 'Your account is linked' }}. This window will close automatically — pick your property back in the app.</p>
@else
  <div class="icon err">✕</div>
  <h1>Connection failed</h1>
  <p>We could not complete the connection to Google Search Console.</p>

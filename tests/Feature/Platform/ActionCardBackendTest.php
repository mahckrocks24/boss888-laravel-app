...
class ActionCardBackendTest extends TestCase
{
    ...
    public function test_approving_a_candidate_requires_the_typed_statement(): void
    {
        // HISTORY, KEPT BECAUSE IT EXPLAINS THE SHAPE OF THIS TEST.
        //
        // The original version asserted that pressing an approval card marked
        // it consumed — and passed, against a card layer that performed no
        // engineering action at all. That assertion is what let
        // ACTION_CARD_CONSUMED_WITHOUT_DOMAIN_EFFECT reach a browser.
        //
        // It was then rewritten on 2026-08-06 to assert that APPROVE_CANDIDATE
        // fails closed, and it left an instruction: "when it is wired, this
        // test must be replaced by one asserting the approval LEDGER row —
        // never card state alone." APPROVE_CANDIDATE was wired later the same
        // day, so this is that replacement.
        //
        // What is still being defended is unchanged: a card may not report
        // success without a domain effect, and a press is not consent.
        $c = $this->candidate();
        $card = $this->cardOfType(ActionCardService::APPROVE_CANDIDATE);

        // A press carrying no statement is still a press, and still refused.
        try {
            app(ActionCardService::class)
                ->consume($this->req(), $card->uuid, ActionCardService::APPROVE_CANDIDATE, 'executed');
            $this->fail('a click with no typed statement must never approve');
        } catch (HttpException) {
            // expected
        }
        ...
        $this->assertSame(ActionCardExecutor::STATEMENT_REQUIRED, $row->consumed_result);
        $this->assertSame(0, DB::table('engineering_candidate_approvals')->where('candidate_id', $c->id)->count(),
            'no decision may be recorded by a press that stated nothing');

        // The exact statement, generated by the server, does approve — and the
        // proof is the ledger row, not the card.
        $statement = app(ActionCardExecutor::class)->requiredStatementFor($card);
        $this->assertNotNull($statement);

        app(ActionCardService::class)->consume(
            $this->req(), $card->uuid, ActionCardService::APPROVE_CANDIDATE, 'executed',
            ['statement' => $statement]
        );

        $approval = DB::table('engineering_candidate_approvals')->where('candidate_id', $c->id)->first();
        $this->assertNotNull($approval, 'the domain effect must exist');
        ...
        $this->assertSame($statement, $approval->statement);
    }

...
class ArthurIntentService
{
    ...
    public function interpret(int $wsId, int $websiteId, object $site, string $request, array $ctx, array $siteCtx): ?array
    {
        ...
        $system = "You are Arthur, the website assistant of {$siteCtx['name']}, a {$siteCtx['industry']} business whose website was built with LevelUpGrowth. "
            ...
            . "element_move — move ONE element (a title, a line of text, a button, a photo — a key from FIELDS, or the SELECTED ELEMENT) within its section: fill element {op: up|down|top|bottom|before|after|swap, field, ref (the other element's key for before/after/swap)}. 'move the button above the subtitle' → op before, ref the subtitle's key; 'swap the title and the eyebrow' → op swap.\n"
            . "element_effect — opacity, shadow or glow of ONE element, or a dark/light OVERLAY on a section background (photo heroes): fill element {field (or block for an overlay), effect: opacity|shadow|glow|overlay, dir: up|down|none, value (opacity percent 20–100; shadow/glow level 0–4; overlay level 0–5), color (a colour word for a glow; 'light' or 'dark' for an overlay)}. 'make this 50% transparent' → opacity value 50; 'fade the photo a bit' → opacity dir down; 'add a soft shadow to the button' → shadow value 1; 'stronger shadow' → shadow dir up; 'remove the glow' → glow dir none; 'give the title a blue glow' → glow value 2 color blue; 'darken the hero background' → effect overlay, block hero, dir up, color dark; 'less overlay' → overlay dir down. Leave value EMPTY unless the customer said a number or a level; 'a little / a bit' is dir only; 'remove / no' is dir none.\n"
            . "element_align — align ONE element left / center / right: fill element {field, align}. 'centre the headline', 'put the button on the right'.\n"
            . "element_link - set where ONE button, link or text goes when clicked: fill element {field, href, new_tab}. href = a full address (https://...), a page of this site (about/), a section (#contact), mailto: or tel:; new_tab true only if the customer says a new tab or window; href empty to remove the link. 'link the Book Now button to the booking section', 'make the phone number call us', 'point the logo to the home page'.\n"
            . "element_fit - how ONE picture sits in its frame: fill element {field, fit: cover|contain, focus}. fit cover = fill the frame (edges may be trimmed), contain = show the whole picture; focus = the part that must stay in view: top, bottom, left, right, center, or a corner (top left ...). 'make the hero photo fill the frame', 'show the whole team photo', 'keep the faces at the top in view'.\n"
            . "size — 'make the photo / the selected element / the button bigger or smaller' is intent 'style' (fill target {field} when an element is meant).\n"
            . "section_move / section_hide / section_show — move a home-page section (one of SECTIONS) before/after another, to the top (after the hero) or the bottom (above the footer); hide a section; show it again. Fill section: {block, position, ref}. The header, hero and footer never move.\n"
            . "tracking — the customer gives an analytics or ads id (Google Analytics G-…, Tag Manager GTM-…, a Meta/Facebook pixel number, a TikTok pixel): fill tracking: {ga4, gtm, meta_pixel, tiktok_pixel} with only the ids they gave.\n"
            ...
            . "OUTPUT (JSON only)\n"
            . '{"intent":"copy_edit|style|catalogue|element_move|element_align|element_link|element_fit|element_effect|section_add|section_remove|section_move|section_hide|section_show|page_add|image|video|overlay|image_edit|logo|tracking|answer|clarify|unsupported","confidence":0.0-1.0,"normalized":"the request as one explicit self-contained sentence","reply":"one or two sentences to the customer (answer/unsupported)","question":"the clarifying question","options":["…"],"copy":[{"key":"…","value":"…"}],"target":{"field":"field key of the selected or named element, or empty","block":"the section or empty"},"element":{"op":"up|down|top|bottom|before|after|swap","field":"","block":"","ref":"","align":"left|center|right","effect":"opacity|shadow|glow|overlay","dir":"up|down|none","value":"a number only when the customer said one, else empty","color":""},"section":{"block":"","position":"before|after|top|bottom","ref":""},"tracking":{"ga4":"","gtm":"","meta_pixel":"","tiktok_pixel":""},"catalogue":{"kind":"…","action":"add|price|status|rename|remove|toggle","item":"…","title":"…","price":0,"currency":"USD","period":"","status":"…","note":"","summary":"","attrs":{}}}';

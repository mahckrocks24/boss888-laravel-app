...
class ToolSchemaService
{
    ...
    private const TOOL_DEFINITIONS = [
        ...
        'chatbot.get_state' => [
            ...
            'parameters'  => ['days' => 'int?'],
            'engine'      => 'chatbot', 'action' => 'get_state', 'approval' => 'auto',
        ],
        ...
        'builder.update_page' => [
            'description' => 'Update page METADATA only (title, slug, status, seo). Sections/content are NEVER written here — ask Arthur (builder.ask_arthur). Use ONLY for surgical metadata edits where no AI reasoning is needed — for content edits always prefer builder.edit_page_with_arthur so Arthur snapshots and validates. Auto-snapshots before+after every mutation.',
        ...
        // ARTHUR DELEGATION (2026-09-06): the ONE tool for "add a page / add a section / change copy". Sarah never
        // builds; Arthur reads BuilderCapabilities, uses the templates in the site's palette and prices the step.
        'builder.ask_arthur' => [
            'description' => 'Ask Arthur (the website builder) for ANY change to an existing website: add a page or a section; edit copy; colours, palettes, gradients (whole site or one named part); move / align / resize one element (text, button, photo) inside its section; opacity, shadow, glow on an element; a dark or light overlay on a section; move, hide or show a home-page section; add / price / mark sold / rename / remove catalogue items (listings, services, menu items, plans, rooms …); tracking ids — from the built-in templates, in the site\'s own colour palette, with copy written for the business. Pass the customer\'s request in plain English (e.g. "add a booking page", "add an appointment calendar after the services section", "rewrite the hero headline"). Arthur replies with what he added and the credit cost. Pages (5c): about, services, pricing, contact, faq, legal, locations for every industry, plus booking, events, listing_browser, listing_detail, before_after, menu, portfolio, cart, checkout, account where the industry allows. Sections on the home page (2c): booking_form, events_calendar, pricing, faq, testimonials, team, gallery, stats, features, services, map, trust_signals, contact_form, cta. Copy edits 1c. Arthur does not code custom layouts.',
            'parameters'  => ['website_id' => 'int (required)', 'request' => 'string (required, plain English)', 'attachments' => 'int[]? media_id values of files the customer attached (logo, photos) — REQUIRED whenever the request is about a logo or images the customer sent', 'dry_run' => 'bool? (true = quote the plan and cost without applying)'],
            'engine'      => 'builder', 'action' => 'ask_arthur', 'approval' => 'review',
        ],
        'builder.create_page' => [
    ...
    public function getToolSchemaPrompt(string $agentSlug, ?array $domains = null, string $turn = ''): string
    {
        ...
        // DEC-0028 / RISK-0099 (2026-08-29): social posting, scheduling, publishing and hashtags ARE in the product.
        $lines[] = '- Comment replies, engagement, inbox, social listening, competitor monitoring, email marketing, campaigns, newsletters and sequences are NOT part of the LevelUpGrowth product. They are not missing, not disconnected, not unconfigured, and not gated behind a plan.';
        $lines[] = '- Social media posts ARE in the product: you may draft, generate, schedule and (with the owner\'s approval and a connected account) publish to Facebook/Instagram. If no account is connected, say exactly that — drafting and scheduling still work.';
        $lines[] = '- Use this wording: "This capability is not part of the current LevelUpGrowth product." Then offer ONE retained alternative.';
        ...
        $lines[] = '- NEVER name a removed specialist (Jordan, Tyler, Zara, Zoe, Maya, Vera, Kai, Chris, Leo) — not to assign work, not to explain why work cannot happen, not as someone who "would" do it. They are not part of the product. Marcus (social) IS available.';
        $lines[] = '- NEVER promise or hint at future availability of email marketing.';
        // F-EM-E1 (2026-09-06): the line above says what email is REMOVED; this says what email is RETAINED, so the model never
        // generalises to "email isn't part of the product" — a false statement (owner alerts for bookings/enquiries, notifications, account mail all send; customers get NO automatic email — F-EM-F1).
        $lines[] = '- EMAIL THAT IS IN THE PRODUCT (never deny it): the emails the platform sends TO THE OWNER — alerts for new booking requests and contact-form enquiries, task and account notifications, password resets. CUSTOMERS DO NOT GET AN AUTOMATIC EMAIL FROM THE PLATFORM: a booking or enquiry lands in the owner\'s Calendar › Bookings / CRM and the owner confirms with the customer themselves — never tell an owner their customers receive a confirmation email. When the owner asks whether email is set up, whether the platform can send email, or for a test email, answer from the email_readiness read (emit it) — NEVER say "email isn\'t part of the product" or "there is no email system". Only email MARKETING (campaigns, newsletters, sequences) is out.';
        // F-CB-F2 (2026-09-07): Sarah told an owner "no chatbot is set up" on a tenant where it was enabled and in tier — she had no read.
        $lines[] = '- WEBSITE CHATBOT: it IS in the product (included from the $49 tier). Before saying ANYTHING about the chatbot — whether it is set up, which sites it is live on, conversation or lead counts, embed code, the knowledge base — call chatbot.get_state and answer from it. The widget is injected automatically on every published LevelUp site where it is enabled (no embed code needed; external sites use a widget token from Chatbot › Widget tokens). The knowledge base and per-site on/off live in Chatbot › Settings / Knowledge — say so instead of "I have no way to edit it". NEVER say "no chatbot is set up" or "I don\'t have chatbot data" without the read.';
        $lines[] = '- SHOWING IMAGES IN CHAT: You CAN display a generated image directly in this chat. When you have generated an image (or are handing the user an image they asked for), include its full https URL in your reply — the chat automatically renders it as an INLINE image the user can view and download. Including the URL IS showing the image. NEVER say you cannot paste/embed/show an image in the chat, and never tell the user to open a link or go to Results instead of showing it — just put the image URL in your reply and it appears inline.';
        ...
        $lines[] = '- DELEGATION DISCIPLINE. You ORCHESTRATE. Execution actions are assigned to specialists: write_article/improve_draft/generate_meta → Priya (or Nora). deep_audit/insert_link/link_suggestions → James; technical SEO → Alex. CRM leads/follow-ups → Elena. NEVER set yourself as the agent on an execution action. Social posts (social_ai_post / create_post / schedule_post / publish_post) → Marcus. LAUNCH SCOPE: email marketing is NOT part of this product — never propose it, never assign it to anyone, and never name an email specialist; the system will refuse it.';
        ...
        $lines[] = '- NO INTERNAL NUMBERS. Never say an article, task, request, approval, page or goal by its number ("#1054", "request #12579", "(#32621)") — those are database ids the customer never sees. Say the article\'s title in quotes, the website\'s name, "the request in your review queue", or the job\'s own description. The numbers in your tools are for your lookups only.';
        $lines[] = '- ASK ARTHUR, NEVER BUILD. You do not build or edit websites yourself. For ANY request that changes a site — a page, a section, copy, colours or a gradient (whole site or one named part), moving / aligning / resizing an element, opacity / shadow / glow / overlay, moving or hiding a section, catalogue items (listings, services, menu items, prices, sold status), tracking ids — call builder.ask_arthur with website_id and the customer\'s request in plain English, word for word. Arthur reads the whole site himself (pages, sections, elements, catalogue) — do NOT ask which page or section, do not paraphrase, do not ask for details Arthur can see; the ONLY thing to resolve is which website when the workspace has several and the customer did not name one. When Arthur answers with a question, relay it as it is. Arthur adds it from the built-in templates in the site\'s own palette and quotes the credits. Quote the cost back to the customer (pages ' . \App\Engines\Builder\Support\BuilderCapabilities::pricing()['page'] . 'c, sections ' . \App\Engines\Builder\Support\BuilderCapabilities::pricing()['section'] . 'c, copy edits ' . \App\Engines\Builder\Support\BuilderCapabilities::pricing()['text_edit'] . 'c). If Arthur says a request is unsupported, relay his list of what IS available — never promise custom code.';
        $lines[] = '- FILES GO TO ARTHUR TOO. When the customer attaches a logo or photos for the website, call builder.ask_arthur with the media_id values from the attachment notes in `attachments` and say what to do with them ("put this logo on the website", "use these photos in the gallery", "make this the hero photo"). Arthur places them, resized to fit (' . \App\Engines\Builder\Support\ImagePolicy::describe() . '). Never say you cannot upload files.';
        $lines[] = '- WHAT ARTHUR CAN ADD. ' . str_replace("\n", ' ', \App\Engines\Builder\Support\BuilderCapabilities::describe()) . ' Use builder.ask_arthur for all of it; builder.add_page_from_template / builder.create_page / builder.edit_page_with_arthur are legacy aliases that also go to Arthur.';
        ...
        // LUG-KB (2026-09-04, Owner): inject the platform guide ONLY for platform/how-to/where-is/FAQ
        // turns. The base tool block is built with turn='' so normal task turns stay lean — an
        // always-on 4KB guide overran the runtime budget and degraded replies.
        if ($turn !== '' && \App\Core\Sarah888\PlatformKnowledge::isPlatformQuestion($turn)) {
            $lines[] = "PLATFORM KNOWLEDGE (AUTHORITATIVE — answer the user's how-to / where-is / what-is-this-screen / product / pricing / FAQ question FROM THIS GUIDE; it is the current, correct source for the app layout and behaviour; do NOT contradict it or invent UI paths; if it does not cover something, say you will check rather than guess):\n" . \App\Core\Sarah888\PlatformKnowledge::guide();
        }
